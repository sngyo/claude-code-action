...
## Fork Changes (sngyo/claude-code-action@beta)
...
- **Updated Base Action**: Uses `sngyo/claude-code-base-action@beta` which includes OAuth credential handling
...
3. Enable OAuth in your workflow:
   ```yaml
   - uses: sngyo/claude-code-action@beta
     with:
       use_oauth: "true"
